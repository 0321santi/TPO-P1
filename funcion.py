# funcion.py (versión corregida sin emojis)
import json
...
def buscar():
    ...
    if productos:
        ...
        print(f"   • Inversion total: ${stats['total_inversion']:.2f}")
        print(f"   • Valor en venta: ${stats['total_valor_venta']:.2f}")
        print(f"   • Ganancia potencial: ${stats['ganancia_neta_total']:.2f} ({stats['ganancia_porcentaje_total']:.2f}%)")
        print(f"   • Productos en stock: {stats['productos_analizados']}")
            ...
            try:
                ...
                for producto in productos:
                    if producto["sku"] == sku:
                        ...
                        print(f"\n PRODUCTO ENCONTRADO:")
                        print(f"   SKU N {sku}: «{producto['nombre']}»")
                        print(f"   Existencias: {producto['existencias']} unidad(es)")
                        print(f"   Precio compra: ${producto['precio compra']:.2f}")
                        print(f"   Precio venta: ${producto['precio venta']:.2f}")
                        print(f"   Categorías: «{categorias_str}»")
                        print(f"\n ANALISIS FINANCIERO:")
                        print(f"   • Inversion en stock: ${producto['precio compra'] * producto['existencias']:.2f}")
                        print(f"   • Valor venta stock: ${producto['precio venta'] * producto['existencias']:.2f}")
...
def ingresar_proveedor():
    ...
    try:
        ...
        # Verificar que el nombre no esté duplicado
        for proveedor in proveedores:
            if normalizar(proveedor["nombre"]) == normalizar(nombre):
...
def gestionar_proveedores_modo(modo=None):
    ...
    if modo == "agregar":
        ...
        while True:
            ...
            print("║ 4. Buscar/Ver proveedores    ║")
            print("║ 5. Análisis de proveedores   ║")
