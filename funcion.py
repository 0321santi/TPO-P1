...
# me voy termine de cambiar la memoria fijate guardalo y hay que cambiar las funciones segun como guarde
...
def initialize_json_file(filepath, default_data):
    """Inicializa un archivo JSON con datos por defecto"""
    if not file_exists(filepath):
        with open(filepath, 'w', encoding='utf-8') as f:
            json.dump(default_data, f, indent=2, ensure_ascii=False)
        escribir_log(f"Archivo {filepath} inicializado", "INFO")


def ensure_system_exists():
    """Asegura que el sistema de archivos esté inicializado"""
    if not file_exists('memory_index.txt'):
        # Crear índice
        with open('memory_index.txt', 'w', encoding='utf-8') as f:
            f.write("productos:data/productos.json\n")
            f.write("categorias:data/categorias.json\n")
            f.write("configuracion:data/configuracion.json\n")

        # Inicializar archivos
        initialize_json_file('data/productos.json', [])
        initialize_json_file('data/categorias.json', [])
        initialize_json_file('data/configuracion.json', {})

        escribir_log("Sistema de archivos inicializado", "INFO")


def load_index():
    """Carga el índice a memoria"""
    global index_mapping
    index_mapping = {}
    try:
        with open('memory_index.txt', 'r', encoding='utf-8') as f:
            for line in f:
                if ':' in line:
                    key, path = line.strip().split(':', 1)
                    index_mapping[key] = path
    except FileNotFoundError:
        escribir_log("Índice no encontrado", "ERROR")
...
def clean_old_cache():
    """Limpia caché antigua basada en tiempo y uso"""
    current_time = datetime.now().timestamp()
    to_remove = []

    for key, timestamp in cache_timestamps.items():
        if (current_time - timestamp > CACHE_EXPIRY_SECONDS or
                len(memory_cache) > MAX_CACHE_SIZE):
    ...
    if to_remove:
        escribir_log(f"Caché limpiada: {len(to_remove)} elementos", "DEBUG")


def get_data(key, fields=None):
    """Obtiene datos con carga selectiva"""
    global memory_cache, cache_timestamps, access_count

    # Verificar caché primero
    if key in memory_cache:
        access_count[key] = access_count.get(key, 0) + 1
        cache_timestamps[key] = datetime.now().timestamp()
        escribir_log(f"Caché hit para {key}", "DEBUG")
        data = memory_cache[key]
        return filter_fields(data, fields) if fields else data

    # Cargar desde disco si no está en caché
    json_path = index_mapping.get(key)
    if json_path and file_exists(json_path):
        try:
            ...
            escribir_log(f"Datos cargados desde disco: {key}", "INFO")
            return filter_fields(data, fields) if fields else data

        except Exception as e:
            escribir_log(f"Error cargando {json_path}: {e}", "ERROR")
            raise FileNotFoundError(f"Error en archivo {key}: {e}")
    else:
        raise FileNotFoundError(f"Archivo para {key} no encontrado")


def filter_fields(data, fields):
    """Filtra solo los campos específicos para ahorrar memoria"""
...
def set_data(key, data, immediate_flush=True):
    """Guarda datos con opción de escritura diferida"""
    global memory_cache, cache_timestamps

    json_path = index_mapping.get(key)
    if not json_path:
        escribir_log(f"Ruta no encontrada para {key}", "ERROR")
        return False
    ...
    try:
        ...
        if immediate_flush:
            # Escritura inmediata a disco
            flush_to_disk(key, data, json_path)
        else:
            # Escritura diferida
            escribir_log(
                f"Datos de {key} almacenados en caché (escritura diferida)", "DEBUG")

        return True

    except Exception as e:
        escribir_log(f"Error guardando {json_path}: {e}", "ERROR")
        return False


def flush_to_disk(key, data, json_path):
    """Escribe datos a disco"""
    try:
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(data, f, indent=2, ensure_ascii=False)

        escribir_log(f"Datos de {key} guardados en disco", "INFO")
    except Exception as e:
        escribir_log(f"Error escribiendo a disco {json_path}: {e}", "ERROR")


def flush_all_pending():
    """Fuerza la escritura de todos los datos en caché a disco"""
    for key, data in memory_cache.items():
        json_path = index_mapping.get(key)
        if json_path:
            flush_to_disk(key, data, json_path)

    escribir_log("Todos los datos pendientes guardados en disco", "INFO")


def get_partial_data(key, filter_func=None, limit=None):
    """Obtiene datos parciales con filtros"""
    full_data = get_data(key)

    if filter_func and callable(filter_func):
        filtered_data = [item for item in full_data if filter_func(item)]
    else:
...
def clear_cache(key=None):
    """Limpia caché específica o completa"""
    global memory_cache, cache_timestamps, access_count
    ...
    escribir_log(
        f"Caché limpiada{' para ' + key if key else ' completamente'}", "INFO")
...
def get_memory_stats():
    ...
    return {
        ...
        "total_accesses": sum(access_count.values()),
        "most_accessed": max(access_count.items(), key=lambda x: x[1]) if access_count else ("N/A", 0)
    }
...
def initialize_memory_system():
    """Inicializa todo el sistema de memoria"""
    ensure_system_exists()
    load_index()
    escribir_log("Sistema de memoria inicializado", "INFO")
...
def cargar_memory():
    """Carga datos esenciales solamente"""
    try:
        ...
        escribir_log(f"Error cargando datos: {e}", "ERROR")
        return {"productos": [], "categorias": []}


def cargar_solo_productos(fields=None):
    """Carga solo productos con campos específicos"""
    return get_data("productos", fields)


def cargar_solo_categorias():
    """Carga solo categorías"""
    return get_data("categorias")


def cargar_productos_filtrados(filter_func=None, limit=None):
    """Carga productos con filtros aplicados"""
    return get_partial_data("productos", filter_func, limit)


def guardar_memory(data):
    """Guarda todos los datos"""
    try:
        success1 = set_data("productos", data.get("productos", []))
        success2 = set_data("categorias", data.get("categorias", []))
        return success1 and success2
    except Exception as e:
        escribir_log(f"Error al guardar datos: {e}", "ERROR")
        return False


def guardar_solo_productos(productos):
    """Guarda solo los productos"""
    return set_data("productos", productos)


def guardar_solo_categorias(categorias):
    """Guarda solo las categorías"""
    return set_data("categorias", categorias)

    """Limpia solo la caché de productos"""
    memory_manager.clear_cache("productos")
    print("Caché de productos limpiada.")
# =============================================================================
...
def buscar():
    try:
        productos = cargar_solo_productos()
        encuentro = False
            ...
            case "4" | "cuatro" | "buscar precio":
                ...
                try:
                    if modo in ("igual", "1", "exacto"):
                        ...
                        lo = float(
                            input("Ingrese el precio mínimo del rango: "))
                        hi = float(
                            input("Ingrese el precio máximo del rango: "))
                        if lo > hi:
                            lo, hi = hi, lo
                        encontrados = [
        ...
        escribir_log(f"Error en función buscar: {e}", "ERROR")
        return False
...
def ingresar():
    try:
        productos = cargar_solo_productos()
        categorias = cargar_solo_categorias()
        ...
        if categorias_input:
            nuevas_categorias = [
                cat.strip() for cat in categorias_input.split(",") if cat.strip()]
            nuevas_categorias = sorted(set(nuevas_categorias))
            print(f"Categorías procesadas: {', '.join(nuevas_categorias)}")
        else:
            nuevas_categorias = []
        ...
        fecha_vencimiento = input(
            "Ingrese fecha de vencimiento (AAAA-MM-DD, 'no' para no perecedero, o Enter para omitir): ").strip()
        if fecha_vencimiento.lower() in ["no", "n", "-1", ""]:
        ...
        # Actualizar categorías si hay nuevas
        for categoria in nuevas_categorias:
            if categoria not in categorias:
                categorias.append(categoria)

        # Guardar usando escritura diferida para mejor rendimiento
        success1 = set_data("productos", productos, immediate_flush=False)
        success2 = set_data("categorias", categorias, immediate_flush=False)

        # Forzar guardado al final
        flush_all_pending()
        ...
        escribir_log(f"Error en función ingresar: {e}", "ERROR")
        return False
...
def ingresar_paquete():
    ...
    try:
        ...
        fecha_vencimiento = input(
            "Ingrese fecha de vencimiento (AAAA-MM-DD o -1 para no poner): ")
        if fecha_vencimiento == "-1":
        ...
        print("¡Error! Ingrese valores numéricos válidos para SKU, existencias, precio y umbral.")
        escribir_log(
            "Error al ingresar valores numéricos en ingreso de producto por paquete.", nivel="ERROR")
        return False
...
def verificar_umbral_minimo():
    """Verifica umbral mínimo - carga solo productos"""
    productos = cargar_solo_productos()
...
def verificar_vencidos():
    ...
    if productos_vencidos:
        ...
        for producto in productos_vencidos:
            print(
                f"«{producto['nombre']}» (SKU: {producto['sku']}) - Vencido el {producto['fecha_vencimiento']}")
        escribir_log(
